# 知账 ZhiZhang — 你的个人财务 AI 顾问

> 别再问"钱都花哪去了"。让 AI 帮你管账，从记账到规划，真正用起来的个人财务助手。
...
1. **规则引擎** — 确定性商户关键词（京东→购物、星巴克→餐饮），秒级匹配
2. **知识图谱（本体）** — 商户别名归一化 + 语义推理，"北京三快""美团点评"自动归到美团
3. **LLM 兜底** — 长尾商户交给大模型，附判断依据和推荐标签，一键确认后自动沉淀到本体，越用越准
...
└───────────────┴───────────────────────────┴─────────────────┘
                 ...
                 ┌───────┴───────┐
                 │  知识图谱层     │
                 │ ontology.yaml  │  模式层（TBox）
                 │ instances.yaml │  实例层（ABox）
                 │ query.py       │  推理接口
                 └───────────────┘
```
...
| 知识图谱 | YAML 本体（TBox） + 实例沉淀（ABox）+ 推理查询 |
